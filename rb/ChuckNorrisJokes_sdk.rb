...
require_relative 'ChuckNorrisJokes_types'
...
class ChuckNorrisJokesSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ChuckNorrisJokesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ChuckNorrisJokesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ChuckNorrisJokesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ChuckNorrisJokesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ChuckNorrisJokesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ChuckNorrisJokesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ChuckNorrisJokesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ChuckNorrisJokesError.new(
        "#{op}_allow",
        "ChuckNorrisJokesSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue ChuckNorrisJokesError => err
    ...
    if fetched.is_a?(Hash)
      status = ChuckNorrisJokesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ChuckNorrisJokesError.new(
        "graphql_error", "ChuckNorrisJokesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ChuckNorrisJokesSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
