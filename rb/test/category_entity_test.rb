...
class CategoryEntityTest < Minitest::Test
  def test_create_instance
    testsdk = ChuckNorrisJokesSDK.test(nil, nil)
  ...
  def test_stream
    ...
    base = ChuckNorrisJokesSDK.test(seed, nil)
    seen = base.Category(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = ChuckNorrisJokesConfig.shared_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = ChuckNorrisJokesSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CHUCK_NORRIS_JOKES_TEST_CATEGORY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def category_basic_setup(extra)
  ...
  client = ChuckNorrisJokesSDK.test(options, extra)
  ...
  entid_env_raw = ENV["CHUCK_NORRIS_JOKES_TEST_CATEGORY_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["CHUCK_NORRIS_JOKES_TEST_CATEGORY_ENTID"])
  ...
  if env["CHUCK_NORRIS_JOKES_TEST_LIVE"] == "TRUE"
    ...
    client = ChuckNorrisJokesSDK.new(Helpers.to_map(merged_opts))
  end
