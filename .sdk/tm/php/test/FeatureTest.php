...
// ChuckNorrisJokes SDK feature test
//
...
class FtCtrl extends ChuckNorrisJokesControl
{
...
class FtClient
{
    ...
    private ChuckNorrisJokesUtility $utility;

    public function __construct(ChuckNorrisJokesUtility $utility, array $options)
    {
    ...
    public function get_utility(): ChuckNorrisJokesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ChuckNorrisJokesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ChuckNorrisJokesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ChuckNorrisJokesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ChuckNorrisJokesConfig::make_feature($name);
    ...
    private static function buildUrl(ChuckNorrisJokesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ChuckNorrisJokesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ChuckNorrisJokesResponse($fetched);
        }
    ...
    private function fail(ChuckNorrisJokesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ChuckNorrisJokesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
