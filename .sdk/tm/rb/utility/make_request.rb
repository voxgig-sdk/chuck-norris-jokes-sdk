...
module ChuckNorrisJokesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ChuckNorrisJokesResponse.new({})
    ...
    if fetch_err
      ...
      response = ChuckNorrisJokesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ChuckNorrisJokesResponse.new(fetched)
    else
