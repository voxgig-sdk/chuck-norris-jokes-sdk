# chuck-norris-jokes-mcp

[MCP](https://modelcontextprotocol.io) server exposing the ChuckNorrisJokes SDK as
two agent tools — `chuck-norris-jokes_list` and `chuck-norris-jokes_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/chuck-norris-jokes-mcp)
make build

# 2. Provide credentials via the environment
export CHUCK_NORRIS_JOKES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user chuck-norris-jokes \
  -- /absolute/path/to/chuck-norris-jokes-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./chuck-norris-jokes-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// chuck-norris-jokes_list: first page of records
{ "entity": "category" }
{ "entity": "category", "query": { } }

// chuck-norris-jokes_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/chuck-norris-jokes-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user chuck-norris-jokes \
     -- "$PWD"/dist/*/chuck-norris-jokes-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `chuck-norris-jokes_list` and `chuck-norris-jokes_load` tools now appear
   in new sessions. Ask the agent to *"list category using chuck-norris-jokes"*
   and it calls `chuck-norris-jokes_list` with `{"entity":"category"}`.
...
```sh
export CHUCK_NORRIS_JOKES_APIKEY=sk_live_xxx            # API key
export CHUCK_NORRIS_JOKES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `chuck-norris-jokes_list` tool
...
### Call the `chuck-norris-jokes_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `chuck-norris-jokes_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `chuck-norris-jokes_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CHUCK_NORRIS_JOKES_APIKEY` | API key sent with every request. |
| `CHUCK_NORRIS_JOKES_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

category | joke | search

### Smoke test via HTTP (raw JSON-RPC)

```sh
./chuck-norris-jokes-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"chuck-norris-jokes_load","arguments":{"entity":"category","query":{"id":1}}}}'
```
