...
require("chuck-norris-jokes_types")
...
local ChuckNorrisJokesSDK = {}
ChuckNorrisJokesSDK.__index = ChuckNorrisJokesSDK
...
ChuckNorrisJokesSDK._make_feature = _make_feature


function ChuckNorrisJokesSDK.new(options)
  local self = setmetatable({}, ChuckNorrisJokesSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ChuckNorrisJokesSDK:options_map()
...
function ChuckNorrisJokesSDK:get_utility()
  return Utility.copy(self._utility)
end


function ChuckNorrisJokesSDK:get_root_ctx()
  return self._rootctx
end


function ChuckNorrisJokesSDK:prepare(fetchargs)
...
function ChuckNorrisJokesSDK:direct(fetchargs)
...
function ChuckNorrisJokesSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function ChuckNorrisJokesSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "ChuckNorrisJokesSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function ChuckNorrisJokesSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ChuckNorrisJokesSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ChuckNorrisJokesSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function ChuckNorrisJokesSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = ChuckNorrisJokesSDK.new(sdkopts)
  sdk.mode = "test"
