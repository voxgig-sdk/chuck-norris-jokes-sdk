...
import (
	...
	omni "github.com/voxgig-sdk/chuck-norris-jokes-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ChuckNorrisJokesError); is {
				out := map[string]any{
					"name":    "ChuckNorrisJokesError",
...
func omniCtx(arg any, client *sdk.ChuckNorrisJokesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ChuckNorrisJokesSDK); is {
			client = live
