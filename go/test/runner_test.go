...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("CHUCK_NORRIS_JOKES_TEST_LIVE") == "TRUE" ||
		os.Getenv("CHUCK_NORRIS_JOKES_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("CHUCK_NORRIS_JOKES_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.ChuckNorrisJokesSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.ChuckNorrisJokesSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.ChuckNorrisJokesError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.ChuckNorrisJokesSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.ChuckNorrisJokesError{Msg: msg, Code: code}
}
