...
//     (\`new ChuckNorrisJokesSDK(...)\` / \`ChuckNorrisJokesSDK.test(...)\` become
//     \`ChuckNorrisJokesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ChuckNorrisJokesSDK'
...
const TEST_SEED = {"entity":{"category":{"test01":{"id":"test01"}},"joke":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ChuckNorrisJokesSDK(...)\` and any
// \`ChuckNorrisJokesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ChuckNorrisJokesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ChuckNorrisJokesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
