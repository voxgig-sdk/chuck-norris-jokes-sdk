...
// ChuckNorrisJokes SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ChuckNorrisJokesTestFeature extends ChuckNorrisJokesBaseFeature
{
    ...
    public function init(ChuckNorrisJokesContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ChuckNorrisJokesContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ChuckNorrisJokesParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ChuckNorrisJokesContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ChuckNorrisJokesContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ChuckNorrisJokesParam::call($ctx, $k);
