...
// ChuckNorrisJokes SDK retry feature
...
class ChuckNorrisJokesRetryFeature extends ChuckNorrisJokesBaseFeature
{
    ...
    public function init(ChuckNorrisJokesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ChuckNorrisJokesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ChuckNorrisJokesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ChuckNorrisJokesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
