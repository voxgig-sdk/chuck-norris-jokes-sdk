...
//    only - a typed ChuckNorrisJokesContext there would make every ctx
...
final class ChuckNorrisJokesOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new ChuckNorrisJokesContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new ChuckNorrisJokesSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new ChuckNorrisJokesResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new ChuckNorrisJokesError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new ChuckNorrisJokesResponse($clean['response']);
    ...
    public static function err_from_map($m): ?ChuckNorrisJokesError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ChuckNorrisJokesError(is_string($code) ? $code : '', $msg);
    }
