...
// ChuckNorrisJokes SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = ChuckNorrisJokesOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(ChuckNorrisJokesOmni::structfix($now, Struct::undef()))
                        !== json_encode(ChuckNorrisJokesOmni::structfix($val, Struct::undef()))) {
