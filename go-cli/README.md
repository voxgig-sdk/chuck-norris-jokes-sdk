...
boru-driven command-line client **and** interactive REPL for the ChuckNorrisJokes
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/chuck-norris-jokes-cli)
make build

# 2. See usage (words, entities, env vars)
./chuck-norris-jokes-cli --help

# 3. Provide credentials once, via the environment
export CHUCK_NORRIS_JOKES_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./chuck-norris-jokes-cli list category
./chuck-norris-jokes-cli list joke

# 5. Override the API base URL for a single call
CHUCK_NORRIS_JOKES_BASE=https://api.example.com ./chuck-norris-jokes-cli list category

# 6. No arguments -> interactive REPL
./chuck-norris-jokes-cli
chuck-norris-jokes> list category
chuck-norris-jokes> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/chuck-norris-jokes-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/chuck-norris-jokes-cli list category
   ```
...
Run with no arguments to open a REPL (prompt `chuck-norris-jokes>`). Each line is
evaluated as its own boru expression:

```text
$ ./chuck-norris-jokes-cli
chuck-norris-jokes> list category
chuck-norris-jokes> /help
chuck-norris-jokes> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/chuck-norris-jokes-cli`. |
...
(`./chuck-norris-jokes-cli <expr>`) and interactively (the REPL), and expressions compose the
